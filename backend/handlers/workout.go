...
func CreateWorkout(c *gin.Context) {
	...
	if strings.TrimSpace(input.Name) == "" || input.DurationMinutes <= 0 || input.CaloriesBurned < 0 || input.Date == "" {
	...
	err = config.DB.QueryRow(
		`INSERT INTO workouts (user_id, name, duration_minutes, calories_burned, date, created_at) 
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
		userID, input.Name, input.DurationMinutes, input.CaloriesBurned, input.Date, time.Now(),
	).Scan(&workoutID)
	if err != nil {
	...
	c.JSON(http.StatusCreated, gin.H{"message": "Workout created successfully"})
}
...
func UpdateWorkout(c *gin.Context) {
	...
	_, err = config.DB.Exec(
		...
		input.Name, input.DurationMinutes, input.CaloriesBurned, input.Date, workoutID, userID,
	)
	if err != nil {
