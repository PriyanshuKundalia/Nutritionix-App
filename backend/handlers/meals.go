...
func (h *Handler) CreateMealFood(c *gin.Context) {
	...
	query := `INSERT INTO meal_foods (id, meal_id, food_name, quantity, calories, protein, carbs, fat) 
              VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`

	if _, err := h.DB.Exec(query, foodID, input.MealID, input.FoodName, input.Quantity, input.Calories, input.Protein, input.Carbs, input.Fat); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "could not add food item"})
		return
	}
...
func (h *Handler) ListMealFoods(c *gin.Context) {
	...
	query := `SELECT id, meal_id, food_name, quantity, calories, protein, carbs, fat FROM meal_foods WHERE meal_id = $1`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&food.ID, &food.MealID, &food.FoodName, &food.Quantity, &food.Calories, &food.Protein, &food.Carbs, &food.Fat); err != nil {
